...
static irqreturn_t st_lsm6ds3_step_counter_trigger_handler(int irq, void *p)
{
	...
	if (!sdata->cdata->reset_steps) {
		printk("%s push step counter value no reset.\n", __func__);
		...
		timestamp = sdata->cdata->timestamp;
	       ...
	       printk("%s push step counter value under reset.\n", __func__);
	...
	printk("%s push step counter value ,step L, H, %d, %d.\n", __func__, *(sdata->buffer_data), *(sdata->buffer_data+1));
