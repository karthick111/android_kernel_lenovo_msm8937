...
#define BMP_MIN_POLL_INTERVAL_MS		100
#define BMP_MAX_POLL_INTERVAL_MS		1000

static struct sensors_classdev pts_dev = {
	.name = "bmp280",
	.vendor = "Bosch",
	.version = 1,
	...
	.min_delay = BMP_MIN_POLL_INTERVAL_MS * 1000,
	.max_delay = BMP_MAX_POLL_INTERVAL_MS,
	.delay_msec = BMP_DELAY_DEFAULT,
	.enabled = 0,
	.sensors_enable = NULL,
			...
			struct device_attribute *attr, char *buf)
{
	...
	for (i = 0; i < REG_CTRL_NUM; i++)
		snprintf(strbuf + strlen(strbuf), PAGE_SIZE, "%02X ", regsbuf[i]);
...
static int bmp_cdev_enable_pts(struct sensors_classdev *sensors_cdev,
						unsigned int enable)
{
	struct bmp_client_data *data =
		container_of(sensors_cdev, struct bmp_client_data, pts_dev);

	mutex_lock(&data->lock);
	pr_err("%s enable = %d\n", __func__, enable);
	if (data->enable != enable) {
		if (enable) {
			sensor_power_config(data->dev, true);
#ifdef CONFIG_PM
			bmp_enable(data->dev);
#endif
			...
			sensor_power_config(data->dev, false);
			cancel_delayed_work_sync(&data->work);
...
static int bmp_cdev_set_pts_delay(struct sensors_classdev *sensors_cdev,
						unsigned int delay_msec)
{
	struct bmp_client_data *data =
		container_of(sensors_cdev, struct bmp_client_data, pts_dev);

	if (data->delay == delay_msec)
		return 0;
...
static int bmp_cdev_pts_flush(struct sensors_classdev *sensors_cdev)
{
	return 0;
}
/*!
...
int bmp_probe(struct device *dev, struct bmp_data_bus *data_bus)
{
	...
	data->pts_dev = pts_dev;
	data->pts_dev.sensors_enable = bmp_cdev_enable_pts;
	data->pts_dev.sensors_poll_delay = bmp_cdev_set_pts_delay;
	data->pts_dev.sensors_flush = bmp_cdev_pts_flush;
	err = sensors_classdev_register(dev, &data->pts_dev);
	if (err) {
		PERR("Sensors class register failed.\n");
		goto error_sysfs;
	}
	...
	PINFO("Successfully probe sensor %s\n", BMP_NAME);
	return 0;
