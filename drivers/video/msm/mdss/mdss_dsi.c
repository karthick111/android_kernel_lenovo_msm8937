...
static int mdss_dsi_panel_power_on(struct mdss_panel_data *pdata)
{
	...
	if (pdata->panel_info.cont_splash_enabled ||
        ...
        pr_debug("[LCD]%s-:cost %lu ms, lp11_init=%d, splash_enable=%d\n", __func__, jiffies_to_msecs(jiffies)-starttimejiffies,
				  ...
				  int event, void *arg)
{
	...
	case MDSS_EVENT_LINK_READY:
		pr_debug("[LCD]%s:Entered Case MDSS_EVENT_LINK_READY\n", __func__);
	...
	case MDSS_EVENT_UNBLANK:
		pr_debug("[LCD]%s:Entered Case MDSS_EVENT_UNBLANK\n", __func__);
		if (ctrl_pdata->on_cmds.link_state == DSI_LP_MODE)
			rc = mdss_dsi_unblank(pdata);
		break;
	case MDSS_EVENT_POST_PANEL_ON:
		pr_debug("[LCD]%s:Entered Case MDSS_EVENT_POST_PANEL_ON\n", __func__);
		rc = mdss_dsi_post_panel_on(pdata);
		break;
	case MDSS_EVENT_PANEL_ON:
		pr_debug("[LCD]%s:Entered Case MDSS_EVENT_PANEL_ON\n", __func__);
	...
	case MDSS_EVENT_BLANK:
		pr_debug("[LCD]%s:Entered Case MDSS_EVENT_BLANK\n", __func__);
		power_state = (int) (unsigned long) arg;
		if (ctrl_pdata->off_cmds.link_state == DSI_HS_MODE)
			rc = mdss_dsi_blank(pdata, power_state);
		break;
	case MDSS_EVENT_PANEL_OFF:
		pr_debug("[LCD]%s:Entered Case MDSS_EVENT_PANEL_OFF\n", __func__);
		power_state = (int) (unsigned long) arg;
		disable_esd_thread();
		ctrl_pdata->ctrl_state &= ~CTRL_STATE_MDP_ACTIVE;
	...
	case MDSS_EVENT_CONT_SPLASH_FINISH:
		pr_debug("[LCD]%s:Entered Case MDSS_EVENT_CONT_SPLASH_FINISH\n", __func__);
		if (ctrl_pdata->off_cmds.link_state == DSI_LP_MODE)
	...
	case MDSS_EVENT_CONT_SPLASH_BEGIN:
		pr_debug("%s: enter case MDSS_EVENT_CONT_SPLASH_BEGIN\n", __func__);
		if (ctrl_pdata->off_cmds.link_state == DSI_HS_MODE) {
